...
- `hermitian_coeffs.py` — pure-numpy utilities to pack a coeffs row of length `N^2` into an `N x N` Hermitian matrix on `Sym^d(C^5)` (`coeffs_row_to_hermitian`, `extract_hermitians`, `monomial_labels`, `_load_coeffs`, `_BLOCK`, `_SYM_DIM`). No JAX dependency. Consumers: `plot_hermitian_coeffs.py` (the CLI), `permute_coeffs.py`, `test_permutation_symmetry.py`, `test_swap_invariance.py`.
