...
def main():
    ...
    # Sanity check: the raw sample should have z[patch] = 1 (the generation
    # pipeline normalises this way). Both calculate_complex_metric_* and
    # compute_holomorphic_form rely on this convention -- their delete_index +
    # insert(patch_idx, 1.0) pattern is *not* a rescaling, it's an autodiff
    # housekeeping step to drop the patch coordinate from the differentiation.
    # Print a quick check so it's obvious if the assumption breaks.
    scale = z_complex[jnp.arange(N), patch_indices]
    print(f"  |z[patch_idx] - 1| on sample:  max={float(jnp.max(jnp.abs(scale - 1.0))):.2e}, "
          f"median={float(jnp.median(jnp.abs(scale - 1.0))):.2e}")
    Q_vals = jnp.abs(jnp.sum(z_complex ** 5, axis=1))
    print(f"  |Σ z^5| on sample (should be ~0 on X):  "
          f"max={float(jnp.max(Q_vals)):.2e}, median={float(jnp.median(Q_vals)):.2e}")
    ...
    # Sanity check: det of a Hermitian matrix is real. If imag/|det| isn't tiny,
    # the Gram matrix isn't actually Hermitian (= pullback formula bug).
    imag_frac_FS = float(np.max(np.abs(np.imag(det_FS_X_c)) / (np.abs(det_FS_X_c) + 1e-30)))
    imag_frac_k4 = float(np.max(np.abs(np.imag(det_k4_X_c)) / (np.abs(det_k4_X_c) + 1e-30)))
    print(f"  Hermiticity check max |imag(det)| / |det|:  FS = {imag_frac_FS:.2e},  k=4 = {imag_frac_k4:.2e}")
    print(f"  ^ should be ~1e-10 or smaller (pure numerical noise).")
    ...
    c_pcts = np.percentile(c_pointwise, [5, 25, 75, 95])
    omega_sq_rescaled = c_mean * omega_sq           # = |Omega'|^2 at each point
    cy_residual = det_k4_X / omega_sq_rescaled
    print(f"  c_pointwise (= det(g_k4|_X) / |Omega|^2) statistics:")
    print(f"    mean   = {c_mean:.4e}")
    print(f"    median = {c_med:.4e}")
    print(f"    std/<c> = {c_std/c_mean:.4f}")
    print(f"    percentiles (5, 25, 75, 95): "
          f"{c_pcts[0]:.4e}, {c_pcts[1]:.4e}, {c_pcts[2]:.4e}, {c_pcts[3]:.4e}")
    # Comparison: same ratio for FS metric. FS is NOT Ricci-flat on X,
    # so its c_pointwise *should* vary even more than the k=4 one. If they're
    # comparable, the variation is being generated by the pullback / |Omega|^2
    # machinery rather than by the metric being non-CY.
    c_FS_pointwise = det_FS_X / omega_sq
    c_FS_pcts = np.percentile(c_FS_pointwise, [5, 50, 95])
    print(f"  Comparison: c_pointwise for FS metric (NOT CY-balanced):")
    print(f"    median = {float(np.median(c_FS_pointwise)):.4e}")
    print(f"    std/<c_FS> = {float(np.std(c_FS_pointwise)/np.mean(c_FS_pointwise)):.4f}")
    print(f"    percentiles (5, 50, 95): "
          f"{c_FS_pcts[0]:.4e}, {c_FS_pcts[1]:.4e}, {c_FS_pcts[2]:.4e}")
    print(f"  ^ if k=4 std/<c> << FS std/<c>, the k=4 metric IS more CY-like.")
    print(f"  ^ if k=4 std/<c> ~ FS std/<c>, the variation is from the pullback")
    print(f"    or |Omega|^2 (a bug), not the metric being non-CY.")
    print()
    print(f"  After rescaling Omega -> sqrt(c) Omega:")
    print(f"    <CY residual det(g_k4|_X) / |Omega'|^2> = {float(np.mean(cy_residual)):.4f}  (= 1 by construction)")
    print(f"    std of CY residual                       = {float(np.std(cy_residual)):.4f}")
